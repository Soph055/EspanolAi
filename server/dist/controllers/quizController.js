...
exports.submitQuiz = exports.createQuiz = void 0;
...
const quizSchema = zod_1.z.object({
    questionCount: zod_1.z.number().int().min(5).max(20).default(10),
});
const submitQuizSchema = zod_1.z.object({
    answers: zod_1.z.array(zod_1.z.object({
        vocabularyId: zod_1.z.number().int().positive(),
        userAnswer: zod_1.z.string().trim().min(1),
        correctAnswer: zod_1.z.string().trim().min(1)
...
const createQuiz = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query(`SELECT id, word, translation FROM vocabulary 
             WHERE user_id = $1
             ORDER BY (times_incorrect - times_correct) DESC, RANDOM()
             LIMIT $2`, [userId, questionCount]);
        ...
        const aiResponse = await (0, gemini_1.generateContent)(prompt, QUIZ_SYSTEM_INSTRUCTION);
        ...
        try {
            ...
            logger_1.default.error("[quizController.createQuiz] failed to parse AI response", { aiResponse, err });
        ...
        if (!quiz.questions || !Array.isArray(quiz.questions)) {
            logger_1.default.error("[quizController.createQuiz] AI response missing questions array", { quiz });
            return res.status(500).json({ message: "AI returned invalid quiz format" });
        }
        return res.status(200).json(quiz);
    }
    catch (err) {
        logger_1.default.error("[quizController.createQuiz]", err);
...
const submitQuiz = async (req, res) => {
    ...
    try {
        ...
        for (const answer of answers) {
            ...
            await client.query(`UPDATE vocabulary SET ${columnToInc} = ${columnToInc} + 1 WHERE id = $1 AND user_id = $2`, [answer.vocabularyId, userId]);
            if (isCorrect) {
                correctCount++;
            }
            else {
        ...
        logger_1.default.error("[quizController.submitQuiz]", err);
